...
class admin_model extends CI_Model
{
   ...
   function get_users_list_rows()
    ...
    {
        $SQL = "SELECT count(*) as total_rows FROM books WHERE is_delete = '1'";
        $queryResult = $this->db->query($SQL);
        $result = $queryResult->row_array();
        return $result['total_rows'];
    }
    ...
    {
        $SQL = "SELECT * FROM books WHERE book_id > 0 AND is_delete = '1'";
    ...
    {
        $SQL = "UPDATE books SET is_delete = '0' WHERE book_id = '".(int)$book_id."'";
        $this->db->query($SQL);
    }
    ...
    function upload_book_front_page($book_id,$old_image='')
    {   
        $this->load->library('Iceimage');
        $iceImage = new iceImage(_DIR_BOOKS); 
        
        if(isset($_FILES["book_front_page"]))
        {
            $tempFile = $_FILES['book_front_page']['tmp_name'];
            $fileInfo = pathinfo($_FILES['book_front_page']['name']);
            $fileName = $fileInfo["filename"];
            $basename = $fileInfo["basename"];
            $extension = $fileInfo["extension"];
            
            $filename = $iceImage->getFilename($_FILES["book_front_page"]['name']);
           ...
           $predFile = _DIR_BOOKS.$mainimage;   
            if(move_uploaded_file($_FILES["book_front_page"]['tmp_name'],$predFile)) 
            {
    ...
    function upload_pdf_book_image($book_id,$old_image='')
    {   
        $this->load->library('Iceimage');
        $iceImage = new iceImage(_DIR_BOOKS); 

        
        if(isset($_FILES["pdf_book_image"]))
        {
            $tempFile = $_FILES['pdf_book_image']['tmp_name'];
            $fileInfo = pathinfo($_FILES['pdf_book_image']['name']);
            $fileName = $fileInfo["filename"];
            $basename = $fileInfo["basename"];
            $extension = $fileInfo["extension"];
            
            $filename = $iceImage->getFilename($_FILES["pdf_book_image"]['name']);
            $FileencryptedFileName = $iceImage->getFileencryptedFileName();
            $mainimage = $FileencryptedFileName.".".$extension;
            
           $predFile = _DIR_BOOKS.$mainimage;   
            if(move_uploaded_file($_FILES["pdf_book_image"]['tmp_name'],$predFile)) 
            {
