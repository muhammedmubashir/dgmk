...
{
  // Local Settings
  define('_DIR_BOOKS',DOC_ROOT.'/dgmk/books/');
  define('_DIR_NAAT',DOC_ROOT.'/dgmk/naats/');
  define('_DIR_ARTICLE',DOC_ROOT.'/dgmk/img/articles/');   
  define('_DIR_PDF_BOOKS',DOC_ROOT.'/dgmk/img/pdf_books/');   
  define('_DIR_GALLERY',DOC_ROOT.'/dgmk/img/gallery/');   
}
else
{
  define('_DIR_BOOKS',DOC_ROOT.'/img/books/');  
  define('_DIR_ARTICLE',DOC_ROOT.'/img/articles/');  
  define('_DIR_PDF_BOOKS',DOC_ROOT.'/img/pdf_books/');  
  define('_DIR_NAAT',DOC_ROOT.'/naats/');
}

define('_URL_BOOKS',_DATA_DIR_PATH.'books/title_page/');
define ('BOOKS_W', 800);
define ('BOOKS_H', 600);
define ('BOOKS_THUMB_W', 300);
define ('BOOKS_THUMB_H', 300);

define('_URL_ARTICLE',_DATA_DIR_PATH.'img/articles/');
    define ('ARTICLE_W', 800);
    define ('ARTICLE_H', 600);
    define ('ARTICLE_THUMB_W', 300);
    define ('ARTICLE_THUMB_H', 300);

define('_URL_PDF_BOOKS',_DATA_DIR_PATH.'img/downloads/');
...
function printDateTimeFormat($datetime)
{
    if($datetime != "")
    {
...
{
    return trim(escape(stripslashes($string)));
}

function print_string($string)
{
    return ucfirst((stripslashes($string)));
}
function escape($v)
{
    if(!is_array($v))
...
function clean_proper_nouns($data)
{

    $words = explode(' ',$data);
    foreach($words as &$w)
    $w = trim($w);
    return preg_replace('/ +/', ' ', preg_replace('/\s\s+/', '  ', html_entity_decode(htmlspecialchars(ucfirst(implode(' ',$words))))));
}
