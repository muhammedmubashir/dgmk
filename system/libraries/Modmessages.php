...
class Modmessages {
    ...
    if(isset($_SESSION['error_message']) && count($_SESSION['error_message']) > 0)
    {
        ...
        if($errors)
        {
            $e_str = '<ul class="">';
            for($i=0; $i<$errors; $i++){
                $e_str .= "<li class='notification undone'>".$this->render_msg($_SESSION['error_message'][$i])."</li>" ;
            }
            $e_str .= "</ul>";
        }
    }
    ...
    if(isset($_SESSION['success_message']) && count($_SESSION['success_message']) > 0)
    {
        ...
        if($succ)
        {
            ...
            for($i=0; $i<$succ; $i++)
            {
                $s_str .= "<li class='notification done'>".$this->render_msg($_SESSION['success_message'][$i])."</li>" ;
            }
            $s_str .= "</ul>";
        }
    }
    ...
    if(isset($_SESSION['info_message']) && count($_SESSION['info_message']) > 0)
    {
        ...
        if($info){
            $i_str = '<ul class="">';
            for($i=0; $i<$info; $i++){
                $i_str .= "<li class='notification information'>".$this->render_msg($_SESSION['info_message'][$i])."</li>" ;
            }
            $i_str .= "</ul>";
        }
    }
